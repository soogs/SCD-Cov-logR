...
# simulation 11-april-2021 #
# same as simulation 17-april-2021, but more replicate datasets per condition
# these results are reported in the paper

# I have opened multiple sessions of R to manually conduct the simulation study with parallelization.
# Thus, this R script is used to conduct the simulation for 90 replicate datasets.
# and then the same script is used to do the simulation for different replicates.
...
source("./simulation 11-may-2021/conditions_making.R") # PATH TO BE CHANGED BY USER
...
repshigh <- rownames(subset(condition_df, dimension == "high"))[1:45]
repslow <- rownames(subset(condition_df, dimension == "low"))[1:45]
...
for (rrr in reps_to_do){ 
  ...
  save("results", file = "./simulation 11-may-2021/results1_11_may_2021.Rdata")
  write.table(x = results, file = "./simulation 11-may-2021/results1_11_may_2021.txt", sep=",")
